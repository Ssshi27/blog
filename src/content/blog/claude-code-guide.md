...
source: "旧博客迁移"
author: "User"
summary: "面向零基础用户的 Claude Code 全景指南：安装登录、斜杠命令、IDE 集成、记忆系统、Hooks、MCP、权限系统与新手最佳实践。"
cover_image: ""
---
...
在终端输入 `claude`，首次使用会弹出浏览器让你登录。支持 Claude 订阅账号、API 控制台账号或云平台凭证。如果要切换账号，在会话中输入 `/login`。
...
**安装**：先装 Claude Code CLI，再装 JetBrains 插件，重启 IDE。
...
- 错误示范："修一下 bug"
- 正确示范："修复登录页的 bug：用户输错密码后页面变空白，应该显示错误提示"
...
- 内容质量评分：23/25
- 事实检查状态：已检查（迁移自旧博客，原文最后更新 2026-06-24）
- 是否需要人工审核：否
- 迁移时间：2026-09-11
